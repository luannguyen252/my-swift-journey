    ...
    var body: some View {
        ZStack(alignment: Alignment(horizontal: .leading, vertical: .top)) {
            ...
            VStack {
                HStack {
                    ...
                    VStack {
                        ...
                        // Text("\(cardBalance.clean(places: 2))đ")
                        Text("\(cardBalance)đ")
            ...
            .onAppear() {
                ...
                self.cardBalance = Int(userCards[selectedIndex].balance) ?? 0
                
                Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
                    if (self.UIState.activeCard != selectedIndex) {
                        self.selectedIndex = self.UIState.activeCard
                        guard let balance = Int(userCards[selectedIndex].balance) else { return }
